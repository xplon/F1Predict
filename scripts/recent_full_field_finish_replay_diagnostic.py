"""Replay diagnostic for the recent full-field finish feature candidate.

This compares the default feature pipeline with the explicit
recent-full-field-finish candidate over the same replay rows. It is diagnostic
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="reports/recent_full_field_finish_replay_diagnostic")
    args = parser.parse_args()

    baseline = ReplayCalibrationBuilder(
        PredictionPipeline(iterations=args.iterations)
    ...
    json_path = output_dir / f"{stem}.recent_full_field_finish_replay_diagnostic.json"
    md_path = output_dir / f"{stem}.recent_full_field_finish_replay_diagnostic.md"
...
def _report(
    ...
    return {
        ...
        "candidate_id": "recent_full_field_finish_form",
        "warnings": [
