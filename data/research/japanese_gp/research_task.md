# Codex Research Brief: Japanese Grand Prix
...
- japanese_gp-f1official-qualifying-antonelli-pole-001: antonelli qualifying_pace positive confidence=0.68 uncertainty=0.28

## Output Contract
Prefer a sources+claims manifest based on `data/research/japanese_gp/research_packet_template.json`. Manual JSONL drafts should go to `data/research/japanese_gp/draft_evidence.jsonl`; the prediction pipeline reads claims only after they are audited and archived under `data/evidence/japanese_gp/packets/`. Each claim must include source_url, published_at, observed_at, metric, direction, magnitude, confidence, uncertainty, evidence_text, reasoning, and review_required.
...
## Search Queries
- Japanese Grand Prix 2026-03-29 F1 practice qualifying race preview
- Japanese Grand Prix 2026-03-29 FIA documents race director notes
- Japanese Grand Prix 2026-03-29 team upgrades Mercedes Ferrari McLaren Red Bull
- Japanese Grand Prix 2026-03-29 weather forecast F1
- Japanese Grand Prix Polymarket winner market rules prices
...
6. Run `f1predict validate-evidence --event japanese_gp --path data/research/japanese_gp/draft_evidence.jsonl` if manually editing drafts.
7. Run `f1predict audit-evidence-sources --event japanese_gp --input data/research/japanese_gp/draft_evidence.jsonl --source-log data/research/japanese_gp/source_log.json`.
8. Archive validated claims with `f1predict ingest-evidence --event japanese_gp --input data/research/japanese_gp/draft_evidence.jsonl --source-log data/research/japanese_gp/source_log.json`.

## Source Snapshot Example
- `f1predict snapshot-source --event japanese_gp --url <url> --source <name> --source-class media --published-at <iso> --knowledge-cutoff 2026-03-29T00:00:00+00:00 --claim-id <claim_id>`

## Batch Archive Example
- `f1predict archive-research-packet --input data/research/japanese_gp/research_packet_template.json --event japanese_gp --knowledge-cutoff 2026-03-29T00:00:00+00:00`

## Final Evidence Archive
- `data/evidence/japanese_gp/packets/`
