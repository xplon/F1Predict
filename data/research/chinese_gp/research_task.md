# Codex Research Brief: Chinese Grand Prix
...
- chinese_gp-f1official-fp1-russell-pace-001: russell race_pace positive confidence=0.68 uncertainty=0.28

## Output Contract
Prefer a sources+claims manifest based on `data/research/chinese_gp/research_packet_template.json`. Manual JSONL drafts should go to `data/research/chinese_gp/draft_evidence.jsonl`; the prediction pipeline reads claims only after they are audited and archived under `data/evidence/chinese_gp/packets/`. Each claim must include source_url, published_at, observed_at, metric, direction, magnitude, confidence, uncertainty, evidence_text, reasoning, and review_required.
...
## Search Queries
- Chinese Grand Prix 2026-03-15 F1 practice qualifying race preview
- Chinese Grand Prix 2026-03-15 FIA documents race director notes
- Chinese Grand Prix 2026-03-15 team upgrades Mercedes Ferrari McLaren Red Bull
- Chinese Grand Prix 2026-03-15 weather forecast F1
- Chinese Grand Prix Polymarket winner market rules prices
...
6. Run `f1predict validate-evidence --event chinese_gp --path data/research/chinese_gp/draft_evidence.jsonl` if manually editing drafts.
7. Run `f1predict audit-evidence-sources --event chinese_gp --input data/research/chinese_gp/draft_evidence.jsonl --source-log data/research/chinese_gp/source_log.json`.
8. Archive validated claims with `f1predict ingest-evidence --event chinese_gp --input data/research/chinese_gp/draft_evidence.jsonl --source-log data/research/chinese_gp/source_log.json`.

## Source Snapshot Example
- `f1predict snapshot-source --event chinese_gp --url <url> --source <name> --source-class media --published-at <iso> --knowledge-cutoff 2026-03-15T00:00:00+00:00 --claim-id <claim_id>`

## Batch Archive Example
- `f1predict archive-research-packet --input data/research/chinese_gp/research_packet_template.json --event chinese_gp --knowledge-cutoff 2026-03-15T00:00:00+00:00`

## Final Evidence Archive
- `data/evidence/chinese_gp/packets/`
