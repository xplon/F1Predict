# Codex Research Brief: Miami Grand Prix
...
- miami_gp-f1official-qualifying-antonelli-pole-001: antonelli qualifying_pace positive confidence=0.68 uncertainty=0.28

## Output Contract
Prefer a sources+claims manifest based on `data/research/miami_gp/research_packet_template.json`. Manual JSONL drafts should go to `data/research/miami_gp/draft_evidence.jsonl`; the prediction pipeline reads claims only after they are audited and archived under `data/evidence/miami_gp/packets/`. Each claim must include source_url, published_at, observed_at, metric, direction, magnitude, confidence, uncertainty, evidence_text, reasoning, and review_required.
...
## Search Queries
- Miami Grand Prix 2026-05-03 F1 practice qualifying race preview
- Miami Grand Prix 2026-05-03 FIA documents race director notes
- Miami Grand Prix 2026-05-03 team upgrades Mercedes Ferrari McLaren Red Bull
- Miami Grand Prix 2026-05-03 weather forecast F1
- Miami Grand Prix Polymarket winner market rules prices
...
6. Run `f1predict validate-evidence --event miami_gp --path data/research/miami_gp/draft_evidence.jsonl` if manually editing drafts.
7. Run `f1predict audit-evidence-sources --event miami_gp --input data/research/miami_gp/draft_evidence.jsonl --source-log data/research/miami_gp/source_log.json`.
8. Archive validated claims with `f1predict ingest-evidence --event miami_gp --input data/research/miami_gp/draft_evidence.jsonl --source-log data/research/miami_gp/source_log.json`.

## Source Snapshot Example
- `f1predict snapshot-source --event miami_gp --url <url> --source <name> --source-class media --published-at <iso> --knowledge-cutoff 2026-05-03T00:00:00+00:00 --claim-id <claim_id>`

## Batch Archive Example
- `f1predict archive-research-packet --input data/research/miami_gp/research_packet_template.json --event miami_gp --knowledge-cutoff 2026-05-03T00:00:00+00:00`

## Final Evidence Archive
- `data/evidence/miami_gp/packets/`
