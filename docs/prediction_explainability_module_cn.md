...
- 模型内部正赛能力分、排位能力分和可靠性估计值只作为风险提示；面向人的回答不能把无事实来源的 seed 先验数值当作解释证据；
- 公开 Markdown、API JSON 和 Codex 追问 prompt 都必须脱敏内部能力分、特征加权影响、原始概率差值和未经来源化解释的赛道指数；
- 排位顺序、赛道类型、天气/安全车/轮胎退化估计值；
- 当前预测包的诊断状态和阻塞项。

尤其是同队车手对比时，解释模块必须说明哪些输入是同队共享的赛车/车队输入，哪些可追溯事实真正支持差异。如果差距主要来自没有来源的静态车手先验，模块要把这标成模型校准风险，而不是把内部分数包装成强事实。
